<template>
  <div class="dedicated-login-root" :style="{ backgroundImage: `url(${bgPortal})` }">
    ...
    <div class="seamless-glass-login-card glass-kasir-border">
      ...
      <div class="card-header-bar">
        ...
        <span class="role-badge-pill pill-kasir">
          🧾 Terminal POS Gate 1
        </span>
      </div>

      <!-- Hero Header for Kasir FO -->
      <div class="role-hero-section">
        <div class="brand-avatar-flex">
          ...
          <div class="role-icon-box avatar-kasir">
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <path d="M4 2v20l2-1 2 1 2-1 2 1 2-1 2 1 2-1 2 1V2l-2 1-2-1-2 1-2-1-2 1-2-1-2 1-2-1Z"/>
              <path d="M16 8h-8M16 12h-8M12 16h-4"/>
            </svg>
          </div>
        </div>

        <div class="role-text-group">
          <h1 class="role-title">Login Kasir Front Office</h1>
          <p class="role-desc">
            Terminal POS Kasir — Penjualan Tiket Masuk Walk-In, Dynamic QR E-Ticket, &amp; Rekonsiliasi Kas Shift.
          </p>
        </div>
      </div>
      ...
      <div class="demo-fill-container">
        <button type="button" class="btn-glass-autofill fill-kasir" @click="fillKasirCredentials">
          <span class="sparkle-icon">⚡</span>
          <span>Auto-fill Kredensial Kasir (Siti Rahmawati)</span>
        </button>
      </div>
      ...
      <form class="glass-login-form" @submit.prevent="handleLogin">
        <div class="form-field-group">
          <label for="kasir-email">Alamat Email Kasir / ID Staf POS</label>
          <div class="dark-input-wrap">
            ...
            <input 
              ...
              placeholder="kasir.gate1@kampungcoklat.id" 
              required
            />
          </div>
        </div>
        ...
        <div class="form-field-group">
          <div class="label-row-split">
            <label for="kasir-password">PIN / Kata Sandi Kasir</label>
            <a href="#" class="forgot-link-gold" @click.prevent="alertHelp">Lupa PIN?</a>
          </div>
          <div class="dark-input-wrap">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="input-icon">
        ...
        <div class="checkbox-remember-row">
          <label class="glass-checkbox-label">
            ...
            <span>Kunci terminal POS pada perangkat kasir ini</span>
          </label>
        </div>

        <button type="submit" class="btn-submit-action btn-submit-kasir" :disabled="isSubmitting">
          <span v-if="isSubmitting" class="spinner-ring"></span>
          <span>{{ isSubmitting ? 'Membuka Sesi Kasir &amp; Laci Kas...' : 'Masuk ke Terminal POS Kasir' }}</span>
        </button>
      </form>

      <div class="card-bottom-footnote">
        <span>Setiap sesi penjualan kasir dicatat otomatis untuk audit rekonsiliasi shift harian.</span>
      </div>
    </div>
  </div>
</template>
...
const email = ref('kasir.gate1@kampungcoklat.id')
const password = ref('password123')
const rememberMe = ref(true)
const showPassword = ref(false)
const isSubmitting = ref(false)

const fillKasirCredentials = () => {
  email.value = 'kasir.gate1@kampungcoklat.id'
...
const handleLogin = () => {
  ...
  setTimeout(() => {
    ...
    navigateTo('/pos/regular')
  }, 400)
}

const alertHelp = () => {
  alert('Untuk bantuan reset PIN kasir, hubungi Supervisor Kasir atau Manager Operasional.')
}
</script>
...
.seamless-glass-login-card {
  ...
  border-radius: 22px;
  padding: 32px 30px;
  box-shadow: 0 25px 60px -10px rgba(0, 0, 0, 0.8), 0 0 35px rgba(242, 151, 39, 0.15);
...
.glass-kasir-border {
  border: 1.5px solid rgba(242, 151, 39, 0.45);
}
...
.pill-kasir {
  ...
  color: #FDE68A;
  border: 1px solid rgba(242, 151, 39, 0.55);
}
...
.fill-kasir:hover {
  background: rgba(242, 151, 39, 0.25);
  border-color: #F29727;
  color: #FFFFFF;
}
...
.glass-input-control:focus {
  ...
  border-color: #F29727;
  box-shadow: 0 0 0 3px rgba(242, 151, 39, 0.25);
}
...
.btn-submit-kasir:hover {
  ...
  box-shadow: 0 8px 24px rgba(242, 151, 39, 0.55);
}
