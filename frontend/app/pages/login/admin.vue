<template>
  <div class="dedicated-login-root" :style="{ backgroundImage: `url(${bgPortal})` }">
    ...
    <div class="seamless-glass-login-card glass-admin-border">
      ...
      <div class="card-header-bar">
        ...
        <span class="role-badge-pill pill-admin">
          🛡️ Super Admin Level 1
        </span>
      </div>

      <!-- Hero Header for Super Admin -->
      <div class="role-hero-section">
        <div class="brand-avatar-flex">
          ...
          <div class="role-icon-box avatar-admin">
        ...
        <div class="role-text-group">
          <h1 class="role-title">Login Super Admin</h1>
          <p class="role-desc">
            Portal Pengaturan Sistem, Tarif Tiket Masuk, Katalog Wahana, Diskon &amp; Hak Akses Staf.
          </p>
        </div>
      </div>
      ...
      <div class="demo-fill-container">
        <button type="button" class="btn-glass-autofill fill-admin" @click="fillAdminCredentials">
          <span class="sparkle-icon">⚡</span>
          <span>Auto-fill Kredensial Super Admin</span>
        </button>
      </div>
      ...
      <form class="glass-login-form" @submit.prevent="handleLogin">
        <div class="form-field-group">
          <label for="admin-email">Alamat Email Super Admin</label>
        ...
        <div class="checkbox-remember-row">
          <label class="glass-checkbox-label">
            ...
            <span>Ingat sesi Super Admin di perangkat ini</span>
          </label>
        </div>

        <button type="submit" class="btn-submit-action btn-submit-admin" :disabled="isSubmitting">
          <span v-if="isSubmitting" class="spinner-ring"></span>
          <span>{{ isSubmitting ? 'Memverifikasi Hak Akses...' : 'Masuk Sebagai Super Admin' }}</span>
        </button>
      </form>

      <div class="card-bottom-footnote">
        <span>Akses Terbatas: Hanya untuk staf Administrator IT resmi Kampung Coklat.</span>
      </div>
    </div>
  </div>
</template>
...
const email = ref('admin@kampungcoklat.id')
...
const fillAdminCredentials = () => {
  email.value = 'admin@kampungcoklat.id'
...
const alertHelp = () => {
  alert('Untuk bantuan reset autentikasi Super Admin, hubungi Administrator Database Utama.')
}
</script>
...
.seamless-glass-login-card {
  ...
  border-radius: 22px;
  padding: 32px 30px;
  box-shadow: 0 25px 60px -10px rgba(0, 0, 0, 0.8), 0 0 35px rgba(16, 185, 129, 0.15);
