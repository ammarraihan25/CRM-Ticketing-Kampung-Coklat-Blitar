<template>
  <div class="dedicated-login-root" :style="{ backgroundImage: `url(${bgPortal})` }">
    ...
    <div class="seamless-glass-login-card glass-manager-border">
      ...
      <div class="card-header-bar">
        ...
        <span class="role-badge-pill pill-manager">
          📊 Operational Management
        </span>
      </div>

      <!-- Hero Header for Operational Manager -->
      <div class="role-hero-section">
        <div class="brand-avatar-flex">
          ...
          <div class="role-icon-box avatar-manager">
        ...
        <div class="role-text-group">
          <h1 class="role-title">Login Operational Manager</h1>
          <p class="role-desc">
            Portal Monitoring Omzet GTV, Evaluasi Performa Shift Kasir, Analisis Member &amp; Demografi Pengunjung.
          </p>
        </div>
      </div>
      ...
      <div class="demo-fill-container">
        <button type="button" class="btn-glass-autofill fill-manager" @click="fillManagerCredentials">
          <span class="sparkle-icon">⚡</span>
          <span>Auto-fill Kredensial Manager Ops</span>
        </button>
      </div>
      ...
      <form class="glass-login-form" @submit.prevent="handleLogin">
        <div class="form-field-group">
          <label for="manager-email">Alamat Email Manager Operasional</label>
          <div class="dark-input-wrap">
            ...
            <input 
              ...
              placeholder="manager.ops@kampungcoklat.id" 
              required
            />
          </div>
        </div>
        ...
        <div class="form-field-group">
          <div class="label-row-split">
            <label for="manager-password">Kata Sandi</label>
        ...
        <div class="checkbox-remember-row">
          <label class="glass-checkbox-label">
            ...
            <span>Ingat sesi Manager di perangkat ini</span>
          </label>
        </div>

        <button type="submit" class="btn-submit-action btn-submit-manager" :disabled="isSubmitting">
          <span v-if="isSubmitting" class="spinner-ring"></span>
          <span>{{ isSubmitting ? 'Memvalidasi Sesi Manager...' : 'Masuk Sebagai Operational Manager' }}</span>
        </button>
      </form>

      <div class="card-bottom-footnote">
        <span>Pengawasan Operasional Bisnis Terpadu Kampung Coklat Blitar.</span>
      </div>
    </div>
  </div>
</template>
...
const email = ref('manager.ops@kampungcoklat.id')
...
const fillManagerCredentials = () => {
  email.value = 'manager.ops@kampungcoklat.id'
...
const alertHelp = () => {
  alert('Untuk bantuan reset autentikasi Manager, hubungi IT Administrator.')
}
</script>
...
.seamless-glass-login-card {
  ...
  border-radius: 22px;
  padding: 32px 30px;
  box-shadow: 0 25px 60px -10px rgba(0, 0, 0, 0.8), 0 0 35px rgba(59, 130, 246, 0.15);
